...
module SimpleTests =
    ...
    let testUnifyParamsWithArguments() =
        ExecutionModule.unifyParamsWithArguments [vp "N"] [va "N2"] |> check "u1" (Some([sv "N"]))
        ExecutionModule.unifyParamsWithArguments [snp 1.] [va "N"] |> check "u2" (Some([sn 1.]))
        ExecutionModule.unifyParamsWithArguments [vp "N"] [sna 1.] |> check "u3" (Some([sn 1.]))
        ExecutionModule.unifyParamsWithArguments [snp 1.] [sna 1.] |> check "u4" (Some([sn 1.]))
        ExecutionModule.unifyParamsWithArguments [snp 1.] [sna 2.] |> check "u5" None
    ...
    let realTest() =
        ...
        ExecutionModule.checkGoal (Goal("f1", [sna 1.; va "Res"])) [pseudoFactorialRule]
        |> check "f1" [[sn 1.; sn 1.]]
        ExecutionModule.checkGoal (Goal("f1", [sna 3.; va "Res"])) [pseudoFactorialRule]
        |> check "f2" [[sn 3.; sn 2.]]
